...
class PostHandler extends BaseHandler {
    ...
    send(req, res) {
        return this.store.create(req)
            .then((File) => {
                const host = process.env.TUS_HOST || req.headers.host;
                const url = this.store.relativeLocation ? `${req.baseUrl || ''}${this.store.path}/${File.id}` : `//${host}${req.baseUrl || ''}${this.store.path}/${File.id}`;
